...
| Feature | Spec Ref | Sequence | Covergroup | Status | Remarks |
|---|---|---|---|---|---|
| Basic write to R/W register | Sec 10, 11 | `axi4l_write_seq`, `axi4l_normal_rw_seq` | addr_region_cg: RW | **PASS** | |
| Write address handshake (AWVALID/AWREADY) | Sec 4, 11 | All write sequences | — | **FAIL** | AWADDR is getting changed even after AW handshake is done |
...
| Write to out-of-range address → DECERR | Sec 9, 20 | `axi4l_decerr_seq` | bresp_cg: DECERR | **FAIL** | Corrupting valid registers when out-of-bounds address is provided |
| Unaligned write address → SLVERR | Sec 10, 20 | `axi4l_unaligned_seq` | bresp_cg: SLVERR | **FAIL** | Write is still happening, mem is getting set to 0 |
| Byte-enable (WSTRB) partial write | Sec 17 | `axi4l_fully_rand` | wstrb_cg | **TBD** | Pending randomized test results |
| Address and data may arrive in any order | Sec 11 | `axi4l_concurrent_seq` | txn_type_cg | **FAIL** | After AW+W in same cycle, data fed into register; read-back gives old data |
...
| Read data correctness (readback after write) | Sec 10 | `axi4l_normal_rw_seq` | — | **FAIL** | Tied to AW+W same cycle failure (gives old data) |
